...
```{r}
box(data, inc, race)
#main = "Bar-and-Whisker Plot of 'Monthly Income' by 'Race'")
```
...
```{r}
qq(data, inc, race)
#main = "Normal Q-Q Plot of 'Monthly Income' by 'Race'", xlab = "Expected Normal"
```
...
```{r}
summary(aov(data$inc ~ factor(data$race)))
```
...
“Using a one-way ANOVA, I reject/fail to reject the null hypothesis that there is no mean difference between groups, in the population, $F(?) = ?, p ? .05$” 
  ...
  + <span style="color:red">“Using one-way ANOVA, I reject the null hypothesis that there is no mean difference between the monthly income of differnt racial categories, in the population, $F(3,16) = 4.043, p \leq .05$”</span> 
