...
#### Example: The Relationship between Shape of a Car's Engine (<span>`vs`</span>) and its Transmission Type (<span>`am`</span>)
...
*The Chi Square test is finicky, in that you cannot see the expected frequencies (and observed frequencies) until you run the Chi Square function* As such, I've included, in the <span style="color:blue">`vannstats`</span> package, a <span style="color:blue">`tab`</span> function which returns the observed and expected frequencies. 
