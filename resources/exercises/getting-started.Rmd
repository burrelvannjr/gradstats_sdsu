...
However, to access these "In Development" packages on GitHub, we need to first `install` and `load` a useful CRAN package that is used by developers. This is the Developer Tools or <span style="color:blue">`devtools`</span> package. 
...
#
Now that the <span style="color:blue">`devtools`</span> package is loaded, you can `install` and `load` my GitHub package: <span style="color:blue">`vannstats`</span> package:
